...
-- Tabelle: segments
-- Stores segment definitions (Strava + local auto-climbs)
CREATE TABLE IF NOT EXISTS segments (
    ...
    state VARCHAR(100),
    country VARCHAR(100),
    source VARCHAR(20) NOT NULL DEFAULT 'strava', -- strava|local
    local_fingerprint VARCHAR(160), -- stable key for local climb grouping
    is_auto_climb BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Tabelle: segment_efforts
-- Stores segment efforts for activities (Strava + local)
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_segments_local_fingerprint ON segments(local_fingerprint) WHERE local_fingerprint IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_segment_efforts_source ON segment_efforts(source);
CREATE UNIQUE INDEX IF NOT EXISTS idx_segment_efforts_local_unique ON segment_efforts(segment_id, activity_id, start_index, end_index) WHERE source = 'local';
...
-- Drop existing views to allow schema updates on existing databases
DROP VIEW IF EXISTS recent_activities CASCADE;
DROP VIEW IF EXISTS activity_summary_by_type CASCADE;
DROP VIEW IF EXISTS activity_summary_by_month CASCADE;
DROP VIEW IF EXISTS gear_usage CASCADE;
...
INSERT INTO schema_migrations (version, name) VALUES ('0001', 'baseline') ON CONFLICT (version) DO NOTHING;
