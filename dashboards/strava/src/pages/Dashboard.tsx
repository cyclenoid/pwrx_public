      ...
      <div className="hidden lg:block w-80 xl:w-96 flex-shrink-0 space-y-4">
        ...
        {recentRunPerformance.sampleCount > 0 && (
          <Link
            ...
            aria-label={t('dashboard.runningPerformance.openTraining')}
          >
            <Card className="border-border/60 bg-card/95 shadow-sm transition-colors hover:border-border hover:bg-card">
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="rounded-full border border-border/50 bg-background/70 px-2 py-0.5 text-[11px] font-medium text-muted-foreground">
                    {t('dashboard.runningPerformance.openTraining')}
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-2.5">
                ...
                <div className="flex items-center justify-between gap-3 text-[11px] text-muted-foreground">
                  <span>{t('dashboard.runningPerformance.compareWindow')}</span>
                  <span className="tabular-nums">
                    {formatNumber(Math.round(recentRunPerformance.totalDistanceKm))} {t('records.units.km')}
                  </span>
                </div>
              </CardContent>
            </Card>
...
function RunningPerformanceTile({
  ...
  return (
    <div className="rounded-lg border border-border/50 bg-background/60 px-3 py-2.5">
      ...
      <div className={`mt-0.5 text-[11px] ${tone}`}>{delta ?? '—'}</div>
    </div>
  )
}
